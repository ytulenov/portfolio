...
const resolvePath = (filePath, baseDir) => {
  ...
  if (imageExtensions.some(ext => filePath.toLowerCase().endsWith(ext))) {
    const resolvedPath = `/images/courseworks/${filePath}`;
...
export default function CourseworkPage({ source, frontmatter, baseDir, params }) { // Added params as a prop
  const components = {
...
export async function getStaticPaths() {
  const mdxDir = path.join(process.cwd(), "public", "courseworks");
...
export async function getStaticProps({ params }) {
  ...
  const mdxPath = path.join(process.cwd(), "public", "courseworks", `${slug}.mdx`);
  ...
  const baseDir = path.posix.dirname(`/courseworks/${slug}`);
